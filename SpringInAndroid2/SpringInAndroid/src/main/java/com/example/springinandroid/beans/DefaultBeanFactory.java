...
public class DefaultBeanFactory implements BeanFactory, BeanDefinitionRegistry, Closeable {
    ...
    protected static Map<String, BeanDefinition> beanDefintionMap = new ConcurrentHashMap<>(256);

    private Map<String, Object> singletonBeanMap = new ConcurrentHashMap<>(256);

    private static Map<Class<?>, Set<String>> typeMap = new ConcurrentHashMap<>(256);
    ...
    private Object getOneArgumentRealValue(Object originalValue) throws Throwable {
        ...
        if (originalValue != null) {
            BeanReference br1 = (BeanReference) originalValue;
            if (originalValue instanceof BeanReference) {//始终为true，考虑直接拿type
                ...
                if (StringUtils.isNotBlank(br.getBeanName())) {
                    ...
                    if(realValue == null){//beanmap中没有相同类型（继承或实现都没有），考虑是不是其他情况
                        String type = br.getType()+"";
                        if(type.contains("Map")){
                            Type type1 = br.getgType();
                            ParameterizedType parameterizedType = (ParameterizedType) type1;
                            Type[] fieldArgTypes = parameterizedType.getActualTypeArguments();
                            Class<?> valueType1 = (Class<?>) fieldArgTypes[1]; // 获取值的类型
                            realValue=getBeansOfType(valueType1);
                        }else if(type.contains("Object[]")){
                            Log.d("test__","暂未考虑，期待你的补充");
